...
async function syncChampions(version) {
    ...
    if (!champions.data) {
        throw new Error('Invalid champion JSON: missing data field');
    }
    ...
    const championData = Object.values(champions.data).map(champ => ({
        version,
        championId: champ.id,
